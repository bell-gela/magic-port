      ...
      <div style={{ position:'absolute', inset:0, pointerEvents:'none', overflow:'hidden' }}>
        {!b ? (<>
          ...
          <svg style={{ position:'absolute', top:'-8%', left:'50%', transform:'translateX(-50%)', width:'90vw', maxWidth:'420px', opacity:0.13 }} viewBox="0 0 400 400">
            <circle cx="200" cy="200" r="170" fill="none" stroke="#4299e1" strokeWidth="1"/><circle cx="200" cy="200" r="130" fill="none" stroke="#ecc94b" strokeWidth="0.7"/><circle cx="200" cy="200" r="90" fill="none" stroke="#4299e1" strokeWidth="0.5"/>
            <line x1="30" y1="200" x2="370" y2="200" stroke="#ecc94b" strokeWidth="0.5"/><line x1="200" y1="30" x2="200" y2="370" stroke="#4299e1" strokeWidth="0.5"/>
          ...
          <svg style={{ position:'absolute', top:'-5%', left:'50%', transform:'translateX(-50%)', width:'100vw', maxWidth:'480px', opacity:0.18 }} viewBox="0 0 400 400">
            <circle cx="200" cy="200" r="175" fill="none" stroke="#a78bfa" strokeWidth="1"/><circle cx="200" cy="200" r="135" fill="none" stroke="#7c3aed" strokeWidth="0.7"/><circle cx="200" cy="200" r="95" fill="none" stroke="#a78bfa" strokeWidth="0.5"/>
...
export default function Home() {
  ...
  useEffect(() => {
    async function fetchHealth() {
      try { const res=await fetch('/api/health'); const data=await res.json(); if(!data.error) setHealth(data); } catch(e){console.error(e);}
      finally{setLoading(false);}
    }
    fetchHealth();
  ...
  const hp=calcHP(health); const cond=getCondition(hp); const hpColor=getHPColor(hp);
